...
function calculateTotalFactor() {
    ...
    // Si no hay beneficiarios, asignamos un valor por defecto (por ejemplo, 1)
    if (beneficiaries.length === 0) {
        totalFactor = 1; // Esto asegura que siempre haya un valor por defecto para totalFactor
    } else {
        // Si hay beneficiarios, calculamos el totalFactor según los datos de los beneficiarios
...
function displayUpdatedPlans() {
    ...
    banmedicaPlans.forEach(plan => {
        ...
        const gesTotal = GES_banmedica * numBeneficiarios;
...
// Mostrar los planes al cargar la página (con valor predeterminado de totalFactor)
document.addEventListener('DOMContentLoaded', function () {
    displayUpdatedPlans();  // Mostrar los planes inicialmente
});
