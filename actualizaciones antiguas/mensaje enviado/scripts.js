            ...
            planElement.innerHTML = `
                <div class="rectangle">
                    ...
                    <div class="plan-footer">
                        ...
                        <a href="${plan.pdfUrl}" class="download-button">Ver Plan</a>
...
document.addEventListener('DOMContentLoaded', () => {
    const modal = document.getElementById('contact-modal'); // Modal de contacto
    const closeModal = document.querySelector('.close-modal'); // Botón para cerrar el modal
    const form = document.querySelector('#contact-modal form'); // Formulario del modal
    const successMessage = document.getElementById('success-message'); // Cuadro de mensaje enviado
    const closeMessageButton = document.getElementById('close-success-message'); // Botón de cerrar el mensaje

    // Mostrar el modal al hacer clic en el botón de contacto
    document.querySelectorAll('.contact-button').forEach(button => {
        button.addEventListener('click', event => {
            event.preventDefault();

            // Obtener valores y mostrarlos en el modal
            const planElement = button.closest('.rectangle');
            const planName = planElement.querySelector('.plan-name').textContent.trim();
            const planValueUF = planElement.querySelector('.plan-price p:nth-child(1)').textContent.trim();
            const planValueCLP = planElement.querySelector('.plan-price p:nth-child(2)').textContent.trim();
            const isapreName = planElement.getAttribute('data-isapre');

            // Insertar valores en los campos ocultos y mostrar en el modal
            document.getElementById('hidden-plan-name').value = planName;
            document.getElementById('hidden-plan-value-uf').value = planValueUF;
            document.getElementById('hidden-plan-value-clp').value = planValueCLP;
            document.getElementById('hidden-isapre-name').value = isapreName;

            document.getElementById('display-plan-name').textContent = planName;
            document.getElementById('display-plan-value-uf').textContent = planValueUF;
            document.getElementById('display-plan-value-clp').textContent = planValueCLP;
    ...
    form.addEventListener('submit', (event) => {
        // Evitar la recarga de la página, pero no bloquear el envío del correo
        event.preventDefault();  // Esto es necesario para que el formulario no recargue la página automáticamente

        // Primero envía el formulario, si tienes configurado backend para manejar el correo
        ...
        .then(response => {
            // Mostrar el cuadro de mensaje de éxito
    ...
    // Cerrar el cuadro de mensaje enviado
